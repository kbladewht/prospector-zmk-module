...
- **YADS2 layout**: Second YADS-style screen (left/right half connection status in the top corners, keyboard name, output status, layer roller showing the current keymap layer highlighted, NerdFont modifiers, battery level per half along the bottom) based on [janpfischer/zmk-dongle-screen](https://github.com/janpfischer/zmk-dongle-screen)
